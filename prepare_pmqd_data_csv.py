#!/usr/bin/env python3
"""
Prepare PMQD dataset for Apollo training using CSV mapping
...
def create_hdf5_dataset_from_csv(csv_path, clean_dir, degraded_dir, output_dir, sr=44100, segment_length=3):
    """
    Create HDF5 dataset from PMQD audio files using CSV mapping
    
    Args:
        csv_path: Path to PMQD CSV file
    ...
    # Get original (clean) and degraded entries
    clean_entries = df[df['degradation_type'] == 'original'].copy()
    degraded_entries = df[df['degradation_type'] != 'original'].copy()
    
    print(f"Found {len(clean_entries)} clean entries and {len(degraded_entries)} degraded entries")
    
    # Create mapping between clean and degraded files
    file_pairs = []
    
    for _, clean_row in clean_entries.iterrows():
        # Find corresponding degraded files with same metadata
        matching_degraded = degraded_entries[
            (degraded_entries['genre'] == clean_row['genre']) &
            (degraded_entries['artist'] == clean_row['artist']) &
            (degraded_entries['title'] == clean_row['title']) &
            (degraded_entries['sample_start'] == clean_row['sample_start'])
        ]
        
        clean_file = Path(clean_dir) / clean_row['sample_filename']
        
        for _, degraded_row in matching_degraded.iterrows():
            degraded_file = Path(degraded_dir) / degraded_row['sample_filename']
            
            if clean_file.exists() and degraded_file.exists():
                file_pairs.append((clean_file, degraded_file))
    
    print(f"Found {len(file_pairs)} matching pairs")
    
    if len(file_pairs) == 0:
        print("No matching pairs found! Check file paths and CSV structure.")
        return None
...
def main():
    parser = argparse.ArgumentParser(description="Prepare PMQD dataset for Apollo using CSV mapping")
    parser.add_argument("--csv_path", type=str, required=True, help="Path to PMQD CSV file")
    ...
    # Create HDF5 dataset
    h5_path = create_hdf5_dataset_from_csv(
        args.csv_path,
        args.clean_dir, 
        args.degraded_dir, 
        args.output_dir, 
    ...
    print("\nTo train Apollo:")
    print(f"python train_apollo.py --h5_path {h5_path} --output_dir ./apollo_training")
